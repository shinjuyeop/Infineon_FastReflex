...
The successful freeze authorizes only the next milestone `SAND_FACTOR_CONDITIONED_MODEL_TRAINING`. `FACTOR_VALIDATION` remains sealed from model inference, training, HNM, normalized 80D/model analysis, and visualization until the future candidate is completely trained and frozen. Historical scientific verdicts remain unchanged, and `FACTOR_CONDITIONED_DATA_DOMAIN_HYPOTHESIS` is still not model-tested.
