...
The recalibrated 176-run matrix creates fresh `MILD_RECALIBRATED_DISCOVERY` and `MILD_RECALIBRATED_CONFIRMATION` splits of 88 each. Counts, moderate/Support mechanics, nine-second duration, physical labels, diversity metrics, and generation gates are unchanged. Confirmation remains sealed until all physical gates and Discovery physical/diversity checks pass and one exact interpretation plus metric hash is frozen. Historical exact reuse, cross-split exact/near overlap, replacement, and backfill must remain zero.
