...
## 16. Controls-recalibrated generation

The design froze `sand_factor_conditioned_development_controls_recalibrated_20260903` with the same 132/66 split and 108 Mild, 36 Moderate, 36 ordinary-Support, and 18 delayed-Support counts. Sand mechanics, coupled factor manifolds, and the Concrete/.25 left-only exception remained unchanged; accumulated anti-reuse constraints used only the pre-authorized boundary-adjacent fresh Sand coordinates. Delayed Support kept its exact successful domain. The canonical factor-conditioned expander accepts explicit source-speed profile maps while preserving its legacy shared-profile form for existing frozen designs.

Before run one, the execution config froze completion, yield, source-speed, manifold, contamination, censor, uniqueness, zero-overlap, and zero-model-output gates. The matrix then completed 198/198 once with no pilot, replacement, backfill, rerun, or model access. Objective-valid yield was 196/198. Ordinary Support was TRAIN 24/24 and VALIDATION 12/12; delayed Support was 12/12 and 6/6; strict Sand was 96/96 and 46/48. All 61 gates passed, producing `SAND_FACTOR_CONDITIONED_DEVELOPMENT_CONTROLS_RECALIBRATED_GENERATION_READY`.

The successful freeze authorized only the next milestone `SAND_FACTOR_CONDITIONED_MODEL_TRAINING`. `FACTOR_VALIDATION` remained sealed from model inference, training, HNM, normalized 80D/model analysis, and visualization until the candidate was completely trained and frozen. Historical scientific verdicts remained unchanged at that boundary, and `FACTOR_CONDITIONED_DATA_DOMAIN_HYPOTHESIS` was not yet model-tested.

## 17. Factor-conditioned model training and one-shot validation

The experiment contract is frozen in `configs/experiment/20260904_sand_factor_conditioned_model_training.yaml`. Before the first optimizer step, it fixed the verified data and model provenance, exact frozen V2 normalizer, Pelvis-only causal 80D feature contract, GRU hidden size 32, threshold `.99`, 5 ms persistence, seeds `20260828`–`20260830`, Round 0 plus three TRAIN-only HNM rounds, candidate-selection rule, one-shot validation authorization, regression gates, and non-effective next action. Fitting used Unified TRAIN, valid V2_TRAIN, and eligible `FACTOR_TRAIN` only; it performed no normalizer refit and no validation-guided selection.

All three final checkpoints were frozen before the authorization record changed the fresh-validation logical seal from `SEALED_FOR_FUTURE_FACTOR_VALIDATION` to `AUTHORIZED_ONCE_FOR_FROZEN_FACTOR_CONDITIONED_CANDIDATE`. The evaluator then opened the split once, loaded 65 eligible payloads once, ran exactly one reference and one candidate pass, and consumed the authorization. The reference and candidate used the same loaded record set. The one invalid record remained metadata-only, and the historical generalization HOLDOUT remained unopened.

Fresh strict-Sand specificity improved from 44/46 to 46/46, false Reflexes fell from 2 to 0, and adverse responses with maximum probability at least `.95` fell from 17/46 to 2/46. These gains did not preserve the required control domain: fresh Support fell from 16/18 to 8/18, including delayed Support from 4/6 to 1/6. On the predeclared historical V2 development regression split, overall Hazard detection fell from 59/64 to 45/64 and Support from 30/30 to 17/30. The final conjunctive verdict is `FACTOR_CONDITIONED_DATA_INTERVENTION_NOT_EFFECTIVE`; no deployment candidate, threshold change, feature change, or architecture change is authorized. The only next scientific action is the read-only `FACTOR_CONDITIONED_DATA_INTERVENTION_FAILURE_AUDIT` over saved artifacts.

## 18. Hazard-boundary audit and physical-gate stop

The follow-up audit uses saved checkpoints and TRAIN sources only. Its diagnostic identity is frozen before replay; 109 runs are actually run-disjoint from prior fitting, and seven delayed-Support monitors are separately disclosed as endpoint-disjoint-only because all delayed TRAIN runs had supplied anchors earlier. The audit reconstructs Round 0–3 loss mass and HNM pressure, compares the reference and every failed checkpoint, and tests input, hidden, logit, head, and temporal separation. It performs zero simulation and optimizer steps and rejects consumed `FACTOR_VALIDATION` before payload loading.

The frozen cause is `TRAINING_OBJECTIVE_SAMPLING_TENSION`. One Track-A hypothesis preserves the exact GRU32/80D/20 ms model, normalizer, seeds, Adam settings, threshold, persistence, shuffling, and three HNM rounds while balancing fixed class mass across positive and negative physical domains. There is no hyperparameter sweep or architecture comparator. The intervention is a predeclared hypothesis, not a trained candidate.

Before any candidate training, the separate 120-run boundary-validation design freezes all identities, physical-yield gates, overlap gates, and model-blindness counters. Generation is exactly one fixed-order pass. Failure of any physical gate requires immediate stop before authorization, model inference, V2_VALIDATION replay, training, candidate freeze, or adaptive repair. The generated corpus failed 8/19 gates and was sealed with open count 0, so candidate families, seeds, optimizer steps, HNM rounds, and model comparisons all remain zero. The cycle verdict is `BOUNDARY_RESOLUTION_INVALID`; final generalization remains `NOT_ESTABLISHED`, and the sole next milestone is `HAZARD_BOUNDARY_VALIDATION_PHYSICAL_REDESIGN`.
