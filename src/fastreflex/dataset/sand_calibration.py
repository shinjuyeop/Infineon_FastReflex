...
def _calibration_result_summary(
    ...
    if first is None:
        ...
        event = support
        event_followup = 0 if event is None else censor - int(event)
        if event is None:
            invalid_reason = "support_not_established"
        elif slip is not None:
            outcome = "DUAL_HAZARD"
        elif i1 is None or int(i1) > int(event):
            invalid_reason = "support_without_i1_ordering"
        elif event_followup < int(contract["support_post_event_followup_ms"]):
            invalid_reason = "insufficient_post_support_observation"
        elif group == "ordinary_support_control":
            expected = f"{row['designed_side']}_ONLY"
            if row["support_event_summary"]["side"] != expected:
                invalid_reason = "support_side_mismatch"
            else:
                outcome = "SUPPORT"
        else:
            clean_touchdowns = int(
                np.count_nonzero(
                    np.any(
                        np.asarray(arrays["target_terrain_touchdown"][: int(i1)]),
                        axis=1,
                    )
                )
            )
            if clean_touchdowns < 2:
                invalid_reason = "delayed_support_has_fewer_than_two_touchdowns"
            elif row["support_event_summary"]["side"] != "LEFT_ONLY":
                invalid_reason = "support_side_mismatch"
            else:
                outcome = "SUPPORT"
    ...
    if np.any(interval):
        ...
        peak_spread = 0.0
    if group == "sand_benign" and outcome != "INVALID":
    ...
    row["intent_match"] = bool(
        row["valid"]
        and (
            (group == "sand_benign" and outcome == "STRICT_BENIGN")
            or (group != "sand_benign" and outcome == "SUPPORT")
        )
    )
