...
The saved-evidence ordinary review froze `sand_factor_conditioned_development_controls_recalibrated_20260903`: 132 TRAIN plus 66 VALIDATION, with Mild 108, Moderate 36, ordinary Support 36, and delayed Support 18. Its one-pass model-blind generation completed 198/198 with no pilot, replacement, backfill, rerun, model output, or historical HOLDOUT read. Objective-valid yield is 196/198: 142 strict Sand, 36 ordinary Support, and 18 delayed Support; the other two retained records are one physical Slip and one post-target fall/censor. Ordinary Support passed at TRAIN 24/24 and VALIDATION 12/12, delayed Support passed at 12/12 and 6/6, and all 61 frozen gates passed. Historical, failed-corpus, pilot, and cross-split exact/forbidden-near overlaps and run-ID reuse are zero. Manifest SHA-256 is `70f850f22507384a50c81bdd065c7d485f2e37cd7e181fda20431ed2fede2d50`, NPZ aggregate SHA-256 is `464348e45b20d8c5adac7965e47233b0cae020adc66e3515209e84ddc9da6e21`, and semantic dataset-freeze SHA-256 is `e397c78d19386732eb54ba388c551a8fe213a6097ba7d0819ff20f7b9b0255f4`. Before the authorized training milestone, `FACTOR_VALIDATION` was model-sealed and only the 132-row `FACTOR_TRAIN` split was eligible for model fitting.

## 13. Controls-recalibrated factor-conditioned development corpus

The current factor-conditioned corpus remains exactly 198 frozen records: 132 `FACTOR_TRAIN` and 66 `FACTOR_VALIDATION`. Training eligibility resolves to 96 strict Sand records, 24 ordinary-Support records, and 12 delayed-Support records. Validation contains 46 strict Sand, 18 Support controls, one actual Slip, and one invalid metadata-only record. No failed corpus, pilot, Confirmation, historical HOLDOUT, or prior `FACTOR_VALIDATION` payload enters fitting or hard-negative mining.

After all candidate checkpoints were frozen, the study-specific authorization consumed `FACTOR_VALIDATION` once. Its loader deserialized the 65 eligible payloads exactly once and retained the invalid row as metadata-only provenance. The split is now permanently consumed development evidence: it cannot be reused for training, HNM, tuning, candidate selection, or another fresh validation claim. The corpus manifest, NPZ aggregate, physical audit, semantic freeze, and validation-seal hashes remain the authoritative identity chain; the completed model result does not mutate them.

## 14. Boundary-resolution validation physical stop

`hazard_boundary_resolution_validation_20260904` is a separate 120-run, model-blind corpus whose only split is `BOUNDARY_RESOLUTION_VALIDATION`. Its pre-simulation matrix contains 48 mild Sand, 12 moderate Sand, 42 ordinary Support, and 18 delayed Support cases, evenly distributing 20 runs across every Concrete/Marble × `.20/.25/.30` source-speed cell. Planned signatures, run IDs, historical exact and forbidden-near coordinates, and cross-role identities had zero overlap; no model-derived field was present.

All 120 simulations ran once in frozen order. The immutable result contains 43 strict benign, 59 Support, 3 Slip, 1 Dual Hazard, and 14 invalid runs; objective-valid yield is 102/120. Ordinary Support passed at 42/42 and delayed Support reached its minimum at 17/18, but strict Sand, mild/moderate Sand, invalid-count, Slip/Dual, source-speed, and factor-manifold gates failed. The physical-signature uniqueness ratio was 102/120 (`.85`) and passed its gate.

The dataset is frozen as `SEALED_FAILED_PHYSICAL_EVIDENCE`. It is not a candidate-selection or model-comparison corpus, and its payload has never been authorized or opened for inference. Replacement, adaptive backfill, rerun, and partial promotion are prohibited. A future `HAZARD_BOUNDARY_VALIDATION_PHYSICAL_REDESIGN` may inspect the saved physical ledger without model access and must create a wholly new independent matrix rather than repair this one.
