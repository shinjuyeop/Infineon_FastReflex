"""Frozen contracts for factor-conditioned data-only model training."""
...
def test_dataset_freeze_and_exact_split_identities(
    ...
    assert sha256_file(DATASET / "physical_audit.json") == dataset[
        "physical_audit_sha256"
    ]
    assert sha256_file(DATASET / "validation_seal.json") == dataset[
        "validation_seal_sha256"
    ]
    assert freeze["FACTOR_DATASET_FREEZE_SHA"] == dataset[
        "dataset_freeze_semantic_sha256"
    ]
    assert freeze["FACTOR_TRAIN_SPLIT_SHA"] == dataset[
        "factor_train_split_sha256"
    ]
    assert freeze["FACTOR_VALIDATION_SPLIT_SHA"] == dataset[
...
def test_frozen_architecture_features_normalizer_and_runtime(
    ...
    assert canonical_sha256(document["architecture"]) == document["reference_model"][
        "architecture_sha256"
    ]
    ...
    assert canonical_sha256(model_v2_anchor_refined_policy()) == document[
        "training_protocol"
    ]["extraction_policy_sha256"]
    assert sha256_file(ROOT / document["normalizer"]["path"]) == document[
        "normalizer"
    ]["sha256"]
...
def test_freeze_precedes_one_shot_validation_and_blocks_retraining() -> None:
    source = Path(
        _load_factor_conditioned_runs.__code__.co_filename
    ).read_text(encoding="utf-8")
    assert source.index("candidate_freeze_sha = sha256_file(candidate_freeze_path)") < (
...
def test_consumed_holdout_and_generalization_validation_stay_protected(
    ...
    assert "Generalization_VALIDATION" in document["historical_evidence_boundary"][
        "forbidden"
    ]
