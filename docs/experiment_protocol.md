...
The next authorized operation is a separate one-shot `MILD_RECALIBRATED_CONFIRMATION` replication of that exact label, rule, preprocessing, pooled scalers, localization direction, and frozen V2. Confirmation still has zero payload deserializations, model inference, 80D/FSR analysis, visualization, and hypothesis testing. An alternate hypothesis cannot be substituted after it opens.
