...
`dataset/sand_mild_calibration.py` owns the later mild-only physical ledger and deterministic expansion/validation of the recalibrated future matrix. Its geometry check treats start, width, exit, and topology jointly; it rejects the predeclared Concrete/.25 right topology and fails closed on historical or cross-split contamination. It loads no model and the future 176-run generator is intentionally outside this design milestone.
