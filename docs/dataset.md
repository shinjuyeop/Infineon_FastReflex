...
The next frozen design identifies `sand_factor_conditioned_development_recalibrated_20260903` but does not generate it. Its 198 fresh planned records comprise `FACTOR_TRAIN` 132 and `FACTOR_VALIDATION` 66; each independently spans Concrete/Marble, `.20/.25/.30`, mild Sand, reduced moderate diagnostic Sand, ordinary Support, and delayed Support. Mild coverage preserves the coupled transition-left/right-single adverse manifold and transition-right/left-single comparison manifold in every nonexception cell; Concrete/.25 remains left-only. The 198 run IDs and signatures are unique, with zero historical, pilot, or cross-split exact/forbidden-near overlap. Future one-pass generation must pass the frozen physical gates before any model path may open.
